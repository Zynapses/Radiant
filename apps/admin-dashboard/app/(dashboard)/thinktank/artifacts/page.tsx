...
// ============================================================================
// RADIANT Admin Dashboard - Artifact Engine Page
// apps/admin-dashboard/app/(dashboard)/thinktank/artifacts/page.tsx
// Version: 4.19.0
//
// Admin interface for managing the Artifact Engine GenUI pipeline.
// ============================================================================
...
import { ArtifactViewer } from '@/components/thinktank/artifact-viewer';
...
export default function ArtifactEnginePage() {
  ...
  const fetchDashboard = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/admin/artifact-engine/dashboard');
  ...
  const handleToggleRule = async (ruleId: string, currentActive: boolean) => {
    try {
      await fetch(`/api/admin/artifact-engine/validation-rules/${ruleId}`, {
        method: 'PUT',
  ...
  const handleAddPackage = async () => {
    ...
    try {
      await fetch('/api/admin/artifact-engine/allowlist', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleRemovePackage = async (packageName: string) => {
    try {
      await fetch(`/api/admin/artifact-engine/allowlist/${encodeURIComponent(packageName)}`, {
        method: 'DELETE',
      });
      fetchDashboard();
    } catch (err) {
    ...
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Required self-correction</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="recent" className="space-y-4">
        ...
        <TabsContent value="recent">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                View and inspect recent artifact generation attempts
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {dashboard.metrics.recentSessions.map((session) => (
                    <TableRow key={session.id}>
                      <TableCell className="max-w-xs truncate">{session.prompt}</TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        {session.reflexion_attempts > 0 ? (
                          <span className="text-yellow-500">{session.reflexion_attempts}</span>
                        ) : (
        ...
        <TabsContent value="rules">
          <Card>
            <CardHeader>
              <CardTitle>Cato Control Barrier Functions (CBFs)</CardTitle>
              <CardDescription>
                Security rules enforced during artifact validation. Disabled rules will not block
                generation.
              </CardDescription>
            </CardHeader>
            <CardContent>
      ...
      {selectedSession && (
        <Dialog open={!!selectedSession} onOpenChange={() => setSelectedSession(null)}>
          <DialogContent className="max-w-6xl h-[80vh]">
            <ArtifactViewer sessionId={selectedSession} onClose={() => setSelectedSession(null)} />
          </DialogContent>
        </Dialog>
      )}
    </div>
