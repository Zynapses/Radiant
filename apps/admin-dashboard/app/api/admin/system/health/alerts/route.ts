...
// GET /api/admin/system/health/alerts - Get active alerts
export const GET = withAuth(async (req: AuthenticatedRequest) => {
  const token = getTokenFromRequest(req);
  
  try {
    const response = await fetch(`${API_BASE}/admin/system/health/alerts`, {
      headers: {
    ...
    console.error('[SystemHealth] Failed to fetch alerts:', error);
