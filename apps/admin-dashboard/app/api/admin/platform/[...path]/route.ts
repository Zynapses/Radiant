 ...
 * Platform API Routes
 * 
 * Proxies requests to the platform admin Lambda handler.
 */
...
async function proxyRequest(
  ...
  const endpoint = `/admin/platform/${path.join('/')}`;
  ...
  try {
    ...
    console.error(`[Platform] ${method} ${endpoint} failed:`, error);
    return NextResponse.json(
      { error: { message: 'Failed to proxy platform request' } },
      { status: 500 }
    );
  }
}
