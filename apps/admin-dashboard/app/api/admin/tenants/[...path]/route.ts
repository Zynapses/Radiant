 ...
 * Tenants API Routes
 * 
 * Proxies requests to the tenants admin Lambda handler.
 */
...
async function proxyRequest(
  ...
  const endpoint = `/admin/tenants/${path.join('/')}`;
  ...
  try {
    ...
    console.error(`[Tenants] ${method} ${endpoint} failed:`, error);
    return NextResponse.json(
      { error: { message: 'Failed to proxy tenants request' } },
      { status: 500 }
    );
  }
}
