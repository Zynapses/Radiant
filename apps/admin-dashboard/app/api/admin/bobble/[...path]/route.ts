 ...
 * Bobble API Routes
 * 
 * Proxies requests to the Bobble admin Lambda handler.
 */
...
async function proxyRequest(
  ...
  const endpoint = `/admin/bobble/${path.join('/')}`;
  ...
  try {
    ...
    console.error(`[Bobble] ${method} ${endpoint} failed:`, error);
    return NextResponse.json(
      { error: { message: 'Failed to proxy bobble request' } },
      { status: 500 }
    );
  }
}
