 ...
 * 
 * Proxies requests to the self-audit admin Lambda handler.
 */
...
async function proxyRequest(
  ...
  const endpoint = `/admin/self-audit/${path.join('/')}`;
  ...
  try {
    ...
    console.error(`[SelfAudit] ${method} ${endpoint} failed:`, error);
    return NextResponse.json(
      { error: { message: 'Failed to proxy self-audit request' } },
      { status: 500 }
    );
  }
}
