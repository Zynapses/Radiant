 ...
 * Proxies requests to the Security admin Lambda handler.
 */
...
async function proxyRequest(
  ...
  const endpoint = `/admin/security/${path.join('/')}`;
  ...
  try {
    ...
    console.error(`[Security] ${method} ${endpoint} failed:`, error);
    return NextResponse.json(
      { error: { message: 'Failed to proxy security request' } },
      { status: 500 }
    );
  }
}
