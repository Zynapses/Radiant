 ...
 * Settings API Routes
 * 
 * Proxies requests to the settings admin Lambda handler.
 */
...
async function proxyRequest(
  ...
  const endpoint = `/admin/settings/${path.join('/')}`;
  ...
  try {
    ...
    console.error(`[Settings] ${method} ${endpoint} failed:`, error);
    return NextResponse.json(
      { error: { message: 'Failed to proxy settings request' } },
      { status: 500 }
    );
  }
}
