...
async function proxyRequest(
  ...
  const endpoint = `/admin/orchestration/${path.join('/')}`;
  ...
  try {
    ...
    console.error(`[Orchestration] ${method} ${endpoint} failed:`, error);
    return NextResponse.json(
      { error: { message: 'Failed to proxy orchestration request' } },
      { status: 500 }
    );
  }
}
