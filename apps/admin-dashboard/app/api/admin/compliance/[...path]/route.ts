 ...
 * Proxies requests to the Compliance admin Lambda handler.
 */
...
async function proxyRequest(
  ...
  const endpoint = `/admin/compliance/${path.join('/')}`;
  ...
  try {
    ...
    console.error(`[Compliance] ${method} ${endpoint} failed:`, error);
    return NextResponse.json(
      { error: { message: 'Failed to proxy compliance request' } },
      { status: 500 }
    );
  }
}
