/**
 * Inference Components API Routes
 * 
 * Proxies requests to the inference components admin Lambda handler.
 */
...
async function proxyRequest(
  ...
  const endpoint = `/admin/inference-components/${path.join('/')}`;
  ...
  try {
    ...
    console.error(`[InferenceComponents] ${method} ${endpoint} failed:`, error);
    return NextResponse.json(
      { error: { message: 'Failed to proxy inference components request' } },
      { status: 500 }
    );
  }
}
