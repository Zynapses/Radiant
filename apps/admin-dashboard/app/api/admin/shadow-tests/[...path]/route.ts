/**
 * Shadow Tests API Routes
 * 
 * Proxies requests to the shadow tests admin Lambda handler.
 */
...
async function proxyRequest(
  ...
  const endpoint = `/admin/shadow-tests/${path.join('/')}`;
  ...
  try {
    ...
    console.error(`[ShadowTests] ${method} ${endpoint} failed:`, error);
    return NextResponse.json(
      { error: { message: 'Failed to proxy shadow-tests request' } },
      { status: 500 }
    );
  }
}
