 ...
 * 
 * Proxies requests to the time machine admin Lambda handler.
 */
...
async function proxyRequest(
  ...
  const endpoint = `/admin/time-machine/${path.join('/')}`;
  ...
  try {
    ...
    console.error(`[TimeMachine] ${method} ${endpoint} failed:`, error);
    return NextResponse.json(
      { error: { message: 'Failed to proxy time-machine request' } },
      { status: 500 }
    );
  }
}
