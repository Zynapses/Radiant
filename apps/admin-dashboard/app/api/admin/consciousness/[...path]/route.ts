/**
 * Consciousness Admin API Routes
 * Proxies requests to the Consciousness admin Lambda handler.
 */
...
async function proxyRequest(
  ...
  const endpoint = `/admin/consciousness/${path.join('/')}`;
  ...
  try {
    ...
    console.error(`[Consciousness] ${method} ${endpoint} failed:`, error);
    return NextResponse.json(
      { error: { message: 'Failed to proxy consciousness request' } },
      { status: 500 }
    );
  }
}
