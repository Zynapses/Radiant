/**
 * Storage Config API Routes
 * 
 * Proxies requests to the storage config admin Lambda handler.
 */
...
async function proxyRequest(
  ...
  const endpoint = `/admin/storage-config/${path.join('/')}`;
  ...
  try {
    ...
    console.error(`[StorageConfig] ${method} ${endpoint} failed:`, error);
    return NextResponse.json(
      { error: { message: 'Failed to proxy storage-config request' } },
      { status: 500 }
    );
  }
}
