/**
 * AGI Orchestration API Routes
 * 
 * Proxies requests to the AGI orchestration admin Lambda handler.
 */
...
async function proxyRequest(
  ...
  const endpoint = `/admin/agi-orchestration/${path.join('/')}`;
  ...
  try {
    ...
    console.error(`[AGIOrchestration] ${method} ${endpoint} failed:`, error);
    return NextResponse.json(
      { error: { message: 'Failed to proxy agi-orchestration request' } },
      { status: 500 }
    );
  }
}
