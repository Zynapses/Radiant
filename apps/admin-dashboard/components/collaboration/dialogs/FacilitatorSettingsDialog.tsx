...
export function FacilitatorSettingsDialog({ open, onOpenChange, session }: FacilitatorSettingsDialogProps) {
  ...
  const enableMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`${API_BASE}/api/thinktank/collaboration/facilitator/enable`, {
        method: 'POST',
      ...
      if (!res.ok) throw new Error('Failed to enable facilitator');
      return res.json();
    },
    onSuccess: () => {
  ...
  const disableMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`${API_BASE}/api/thinktank/collaboration/sessions/${session.id}/facilitator`, {
        method: 'DELETE',
      });
      if (!res.ok) throw new Error('Failed to disable facilitator');
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['enhanced-session', session.id] });
      onOpenChange(false);
