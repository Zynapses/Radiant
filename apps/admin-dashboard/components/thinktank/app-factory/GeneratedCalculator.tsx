...
const FORMULAS: Record<string, (inputs: Record<string, number>) => Record<string, number | string>> = {
  ...
  generic: (inputs) => {
    ...
    let result = value1;
    if (operation === 1) result = value1 + value2; // add
    else if (operation === 2) result = value1 - value2; // subtract
    else if (operation === 3) result = value1 * value2; // multiply
    else if (operation === 4) result = value2 !== 0 ? value1 / value2 : 0; // divide
    return { result };
  },
};
