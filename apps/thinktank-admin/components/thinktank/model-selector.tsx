...
export function ModelSelector({
    ...
    const fetchData = async () => {
      try {
        const [modelsRes, prefsRes] = await Promise.all([
          fetch('/api/thinktank/models'),
          fetch('/api/thinktank/preferences/models'),
        ]);
  ...
  const toggleFavorite = async (modelId: string) => {
    try {
      await fetch('/api/thinktank/preferences/models/favorite', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ modelId }),
      });
      // Refetch preferences
      const res = await fetch('/api/thinktank/preferences/models');
          ...
          {showCost && (
            <>
              ...
              <span className="text-green-600">
                $
                {((model.userInputPrice + model.userOutputPrice) / 2).toFixed(2)}
                /1M
              </span>
            </>
          )}
