...
export function RejectionNotifications() {
  ...
  const [rejectionData, setRejectionData] = useState<RejectionDisplayData>({
    hasRejections: false,
    rejections: [],
    unreadCount: 0,
  });
  ...
  const fetchRejections = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setRejectionData(data ?? { hasRejections: false, rejections: [], unreadCount: 0 });
      }
    } catch (error) {
  ...
  const markAsRead = async (notificationId: string) => {
    try {
      await fetch(`${API_BASE}/api/thinktank/rejections/${notificationId}/read`, { method: 'PATCH' });
      fetchRejections();
    } catch (error) {
  ...
  const dismiss = async (notificationId: string) => {
    try {
      await fetch(`${API_BASE}/api/thinktank/rejections/${notificationId}/dismiss`, { method: 'DELETE' });
      fetchRejections();
    } catch (error) {
