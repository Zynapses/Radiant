...
const STATE_CONFIG: Record<AIState, {
  ...
  idle: { icon: Brain, label: 'Ready', color: 'text-muted-foreground', bgColor: 'bg-muted' },
  listening: { icon: MessageSquare, label: 'Listening...', color: 'text-blue-500', bgColor: 'bg-blue-500/10', animation: 'breathe' },
  thinking: { icon: Search, label: 'Thinking...', color: 'text-purple-500', bgColor: 'bg-purple-500/10', animation: 'pulse' },
  pondering: { icon: Lightbulb, label: 'Pondering...', color: 'text-amber-500', bgColor: 'bg-amber-500/10', animation: 'wiggle' },
  generating: { icon: Sparkles, label: 'Generating...', color: 'text-pink-500', bgColor: 'bg-pink-500/10', animation: 'spin' },
  verifying: { icon: Target, label: 'Verifying...', color: 'text-cyan-500', bgColor: 'bg-cyan-500/10', animation: 'pulse' },
  confident: { icon: Check, label: 'Confident', color: 'text-green-500', bgColor: 'bg-green-500/10' },
  uncertain: { icon: AlertTriangle, label: 'Uncertain', color: 'text-amber-500', bgColor: 'bg-amber-500/10', animation: 'wiggle' },
  excited: { icon: Zap, label: 'Excited!', color: 'text-yellow-500', bgColor: 'bg-yellow-500/10', animation: 'bounce' },
  focused: { icon: Eye, label: 'Focused', color: 'text-indigo-500', bgColor: 'bg-indigo-500/10' },
};
...
const pulseAnimation = { scale: [1, 1.1, 1], transition: { repeat: Infinity, duration: 1.5 } };
const breatheAnimation = { scale: [1, 1.05, 1], opacity: [0.8, 1, 0.8], transition: { repeat: Infinity, duration: 3, ease: 'easeInOut' } };
const wiggleAnimation = { rotate: [-5, 5, -5], transition: { repeat: Infinity, duration: 0.5 } };
const bounceAnimation = { y: [0, -5, 0], transition: { repeat: Infinity, duration: 0.6 } };
    ...
    <TooltipProvider>
      ...
      >
        <div className="relative">
          ...
          {affect && (
            <motion.div className="absolute -bottom-1 -right-1 text-sm" initial={{ scale: 0 }} animate={{ scale: 1 }} key={getAffectEmoji(affect)}>
        ...
        <div className="flex flex-col min-w-0">
          <div className="flex items-center gap-2">
            <span className={cn('text-sm font-medium', config.color)}>{config.label}{dots}</span>
            {modelName && <Badge variant="outline" className="text-xs h-5">{modelName}</Badge>}
          </div>
          {currentTask && <p className="text-xs text-muted-foreground truncate max-w-[200px]">{currentTask}</p>}
        </div>
        ...
        <div className="flex items-center gap-3 ml-2 pl-3 border-l border-border/50">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="flex flex-col items-center">
                  <span className="text-xs font-medium">{Math.round(affect.confidence * 100)}%</span>
          ...
          {thinkingDuration !== undefined && thinkingDuration > 0 && (
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="flex flex-col items-center">
                  <span className="text-xs font-medium tabular-nums">{(thinkingDuration / 1000).toFixed(1)}s</span>
          ...
          {tokensGenerated !== undefined && tokensGenerated > 0 && (
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="flex flex-col items-center">
                  <span className="text-xs font-medium tabular-nums">{tokensGenerated}</span>
