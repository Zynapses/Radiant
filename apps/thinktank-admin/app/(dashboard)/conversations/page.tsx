...
export default function ConversationsPage() {
  ...
  const { data: stats } = useQuery<ConversationStats>({
    ...
    queryFn: () => api.get<ConversationStats>('/api/admin/thinktank/conversations/stats'),
  });

  const { data: messages } = useQuery<{ data: ConversationMessage[] }>({
    queryKey: ['conversation-messages', selectedConversation],
    queryFn: () =>
      api.get<{ data: ConversationMessage[] }>(
        `/api/admin/thinktank/conversations/${selectedConversation}/messages`
      ),
  ...
  const createShareLink = async () => {
    ...
    try {
      const data = await api.post<{ shareUrl: string }>('/api/thinktank/share', {
