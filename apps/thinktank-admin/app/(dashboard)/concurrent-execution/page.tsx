...
export default function ConcurrentExecutionPage() {
  ...
  const loadConfig = async () => {
    try {
      const data = await api.get<ConcurrentExecutionConfig>('/api/thinktank/concurrent/config');
      setConfig(data);
    } catch (error) {
  ...
  const loadMetrics = async () => {
    try {
      const data = await api.get<ConcurrentMetrics>('/api/thinktank/concurrent/metrics?period=day');
      setMetrics(data);
    } catch (error) {
  ...
  const saveConfig = async () => {
    setSaving(true);
    try {
      await api.put('/api/thinktank/concurrent/config', config);
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground mb-2">
                    Max queued tasks: {config.maxQueueDepth}
                  </p>
                  <Slider
