# Cato Global Consciousness Infrastructure
# Terraform module for deploying all Cato components
...
resource "aws_dynamodb_table" "semantic_memory" {
  name             = "cato-semantic-memory-${var.environment}"
...
resource "aws_dynamodb_table" "working_memory" {
  name             = "cato-working-memory-${var.environment}"
  billing_mode     = "PAY_PER_REQUEST"
  hash_key         = "pk"
  range_key        = "sk"
...
resource "aws_dynamodb_table" "config" {
  name         = "cato-config-${var.environment}"
  billing_mode = "PAY_PER_REQUEST"
  hash_key     = "pk"
  range_key    = "sk"
...
resource "aws_dynamodb_table" "costs" {
  name         = "cato-costs-${var.environment}"
  billing_mode = "PAY_PER_REQUEST"
  hash_key     = "pk"
  range_key    = "sk"
...
resource "aws_dax_cluster" "semantic_cache" {
  ...
  cluster_name       = "cato-dax-${var.environment}"
...
resource "aws_dax_subnet_group" "main" {
  ...
  name       = "cato-dax-subnet-${var.environment}"
...
resource "aws_elasticache_replication_group" "semantic_cache" {
  replication_group_id       = "cato-cache-${var.environment}"
...
resource "aws_elasticache_parameter_group" "valkey" {
  family = "valkey7"
  name   = "cato-valkey-${var.environment}"
...
resource "aws_elasticache_subnet_group" "main" {
  name       = "cato-cache-subnet-${var.environment}"
  subnet_ids = module.vpc.private_subnet_ids
}

resource "aws_security_group" "cache" {
  name        = "cato-cache-${var.environment}"
...
resource "aws_opensearchserverless_collection" "episodic_memory" {
  name = "cato-episodic-${var.environment}"
  type = "VECTORSEARCH"
...
resource "aws_opensearchserverless_security_policy" "episodic_encryption" {
  name = "cato-episodic-encryption-${var.environment}"
  type = "encryption"
  policy = jsonencode({
    Rules = [
      {
        Resource     = ["collection/cato-episodic-${var.environment}"]
...
resource "aws_opensearchserverless_security_policy" "episodic_network" {
  name = "cato-episodic-network-${var.environment}"
  type = "network"
  policy = jsonencode([
    {
      Rules = [
...
resource "aws_opensearchserverless_vpc_endpoint" "main" {
  name               = "cato-opensearch-${var.environment}"
  vpc_id             = module.vpc.vpc_id
  subnet_ids         = module.vpc.private_subnet_ids
...
resource "aws_neptune_cluster" "knowledge_graph" {
  cluster_identifier                  = "cato-graph-${var.environment}"
  engine                              = "neptune"
...
resource "aws_neptune_subnet_group" "main" {
  name       = "cato-neptune-${var.environment}"
  subnet_ids = module.vpc.private_subnet_ids
...
resource "aws_kinesis_stream" "interactions" {
  name             = "cato-interactions-${var.environment}"
  retention_period = 24
  ...
  encryption_type = "KMS"
  kms_key_id      = aws_kms_key.cato.arn

  tags = local.common_tags
}

resource "aws_kinesis_stream" "curiosity" {
  name             = "cato-curiosity-${var.environment}"
  retention_period = 72
...
resource "aws_kms_key" "cato" {
  description             = "KMS key for Cato encryption"
...
resource "aws_kms_alias" "cato" {
  name          = "alias/cato-${var.environment}"
  target_key_id = aws_kms_key.cato.key_id
}
...
resource "aws_security_group" "lambda" {
  name        = "cato-lambda-${var.environment}"
...
resource "aws_security_group" "eks_nodes" {
  name        = "cato-eks-nodes-${var.environment}"
