...
resource "aws_ecr_repository" "shadow_self" {
  name                 = "bobble-shadow-self-${var.environment}"
  ...
  encryption_configuration {
    encryption_type = "KMS"
    kms_key         = aws_kms_key.bobble.arn
  }
...
resource "aws_ecr_repository" "nli_model" {
  name                 = "bobble-nli-${var.environment}"
  image_tag_mutability = "MUTABLE"
...
resource "aws_s3_bucket" "models" {
  bucket = "bobble-models-${var.environment}-${data.aws_caller_identity.current.account_id}"
...
resource "aws_s3_bucket_server_side_encryption_configuration" "models" {
  ...
  rule {
    apply_server_side_encryption_by_default {
      kms_master_key_id = aws_kms_key.bobble.arn
...
resource "aws_iam_role" "sagemaker" {
  name = "bobble-sagemaker-${var.environment}"
    ...
    Statement = [
      {
        ...
        Resource = [aws_kms_key.bobble.arn]
      }
    ]
  })
}
...
resource "aws_sagemaker_endpoint_configuration" "shadow_self" {
  name = "bobble-shadow-self-config-${var.environment}"
...
resource "aws_sagemaker_endpoint_configuration" "nli" {
  name = "bobble-nli-config-${var.environment}"
...
resource "aws_sagemaker_endpoint" "nli" {
  name                 = "bobble-nli-mme-${var.environment}"
...
resource "aws_cloudwatch_metric_alarm" "shadow_self_latency" {
  alarm_name          = "bobble-shadow-self-high-latency-${var.environment}"
...
resource "aws_cloudwatch_metric_alarm" "shadow_self_errors" {
  alarm_name          = "bobble-shadow-self-errors-${var.environment}"
  comparison_operator = "GreaterThanThreshold"
...
resource "aws_sns_topic" "alerts" {
  ...
  name = "bobble-alerts-${var.environment}"
