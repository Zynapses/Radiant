...
import { queryFallbackService } from '../shared/services/cato/query-fallback.service';
...
export const handler: APIGatewayProxyHandler = async (event) => {
  ...
  try {
    ...
    logger.error('Cato Genesis API error', { error, path: event.path });
    
    if (error instanceof Error && error.message?.includes('requires role')) {
