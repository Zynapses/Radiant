# Bobble Migration Runbook: LiteLLM → vLLM + Ray Serve
...
docker build -t bobble-shadow-self:v1 .

# Push to ECR
aws ecr get-login-password | docker login --username AWS --password-stdin $ECR_REPO
docker push $ECR_REPO/bobble-shadow-self:v1
...
```bash
# Test endpoint
curl -X POST https://runtime.sagemaker.us-east-1.amazonaws.com/endpoints/bobble-shadow-self-staging/invocations \
  -H "Content-Type: application/json" \
  -d '{"inputs": "Hello, Bobble!"}'
```
...
# Deploy Bobble orchestrator
kubectl apply -f k8s/ray-serve/bobble-orchestrator.yaml
```
...
# Load test
locust -f tests/load/locustfile.py --host=https://staging.bobble.thinktank.ai
```
...
async def test_routing():
    orchestrator = BobbleOrchestrator()
...
aws dynamodb describe-table --table-name bobble-semantic-memory \
...
aws appconfig create-hosted-configuration-version \
  --application-id bobble \
  --configuration-profile-id orchestration \
  --content '{"use_new_orchestrator": {"percentage": 10}}'

# Monitor
watch -n 5 'curl -s https://api.bobble.thinktank.ai/api/admin/bobble/health'
```
...
# Verify
curl https://api.bobble.thinktank.ai/api/admin/bobble/status
```
...
kubectl scale deployment litellm -n bobble --replicas=0

# After 1 week of monitoring, delete
kubectl delete deployment litellm -n bobble
...
```bash
# Scale up LiteLLM
kubectl scale deployment litellm -n bobble --replicas=10

# Disable new orchestrator
kubectl scale deployment bobble-orchestrator -n bobble --replicas=0
