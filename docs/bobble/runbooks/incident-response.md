# Bobble Incident Response Runbook
...
- Custom metrics → Slack #bobble-alerts
...
2. Join #bobble-incident Slack channel
...
**Symptoms:**
- 5XX errors from `/api/admin/bobble/shadow-self/*`
...
aws logs filter-log-events \
  --log-group-name /aws/sagemaker/Endpoints/bobble-shadow-self \
...
aws sagemaker update-endpoint \
  ...
  --endpoint-config-name bobble-shadow-self-config
...
aws cloudwatch get-metric-statistics \
  ...
  --dimensions Name=EndpointName,Value=bobble-shadow-self \
...
kubectl scale deployment bobble-orchestrator -n bobble --replicas=50
...
```bash
# Check cache stats
redis-cli -h bobble-cache.xxx.use1.cache.amazonaws.com INFO stats

# Check for recent cache invalidations
aws cloudwatch get-metric-statistics \
  --namespace Custom/Bobble \
...
curl -H "Authorization: Bearer $TOKEN" \
  https://api.bobble.thinktank.ai/api/admin/bobble/budget/status
```
...
aws cloudwatch get-metric-statistics \
  ...
  --dimensions Name=TableName,Value=bobble-semantic-memory
```
...
aws dynamodb update-table \
  --table-name bobble-semantic-memory \
