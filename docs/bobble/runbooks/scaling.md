# Bobble Scaling Runbook

## Overview

This runbook covers scaling Bobble infrastructure to handle increased load.
...
**Scale Up:**
```bash
kubectl scale deployment bobble-orchestrator -n bobble --replicas=100
```

**Check Status:**
```bash
kubectl get pods -n bobble -l app=bobble-orchestrator
```

**Update Auto-Scaling:**
```bash
kubectl patch hpa bobble-orchestrator-hpa -n bobble \
...
aws elasticache modify-replication-group \
  --replication-group-id bobble-cache-production \
...
aws dynamodb update-table \
  --table-name bobble-semantic-memory-production \
...
- **Bobble Overview**: All key metrics
