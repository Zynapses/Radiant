# Bobble Shadow Self GPU Infrastructure

This document describes the GPU infrastructure requirements for running the Shadow Self component of Bobble's consciousness verification system.

## Overview

The Shadow Self is a local neural network that provides mechanistic verification of introspective claims. In production, this uses **Llama-3-8B** running on GPU infrastructure. Currently, Bobble simulates this via LLM API calls, but true local inference provides:
...
```
┌─────────────────┐     ┌──────────────────────┐
│  Bobble Lambda  │────▶│  SageMaker Endpoint  │
│  (Node.js)      │     │  (Llama-3-8B)        │
└─────────────────┘     │  g5.xlarge           │
                        └──────────────────────┘
```
...
```typescript
// packages/infrastructure/lib/stacks/bobble-gpu-stack.ts

import * as sagemaker from 'aws-cdk-lib/aws-sagemaker';

const shadowSelfEndpoint = new sagemaker.CfnEndpoint(this, 'ShadowSelfEndpoint', {
  endpointName: 'bobble-shadow-self',
...
const shadowSelfConfig = new sagemaker.CfnEndpointConfig(this, 'ShadowSelfConfig', {
  endpointConfigName: 'bobble-shadow-self-config',
  productionVariants: [{
...
```
┌─────────────────┐     ┌──────────────────────┐
│  Bobble Lambda  │────▶│  EC2 GPU Instance    │
...
const shadowSelfModel = new sagemaker.CfnModel(this, 'ShadowSelfModel', {
  modelName: 'bobble-shadow-self-llama3',
...
SHADOW_SELF_ENDPOINT=bobble-shadow-self
...
When GPU infrastructure is unavailable, Bobble falls back to:
...
This ensures Bobble remains functional even without dedicated GPU resources.
